...
clear all
close all
Img_Path=dir('./Big_Img_Train\background/*.tif');
step=80;
New_Width=128;
New_Height=128;
tt=1;
...
%             imshow(Img_Crop);
            imwrite(Img_Crop,strcat('./data/temp_file_train/background/',num2str(tt),'.jpg'));
%             pause(0.2);
...
% 
% 
% Img_Path=dir('./Big_Img_Train\muscle/*.tif');
% tt=1;
% figure;
% for i=1:length(Img_Path)
%     Center_x=ceil(New_Width/2)+1;
...
%             Img_Crop=Img(Center_y-floor(New_Height/2):Center_y+floor(New_Height/2)-1,Center_x-floor(New_Width/2):Center_x+floor(New_Width/2)-1,:);
% %             imshow(Img_Crop);
%             imwrite(Img_Crop,strcat('./data/temp_file_train/muscle/',num2str(tt),'.jpg'));
% %             pause(0.2);
%             Center_x=Center_x+step;
%             tt=tt+1;
%         end
...
% clear all
% close all
% Img_Folder=dir('./data/temp_file_train/');
% 
% train_Index=0;
% val_Index=0;
% test_Index=0;
...
%         if  ismember(i,train)
%              imwrite(Img,strcat('./data/train/',num2str(train_Index),'_',Img_Folder(j).name,'.jpg'));
%                 train_Index=train_Index+1;
%         elseif ismember(i,val)
%             imwrite(Img,strcat('./data/val/',num2str(val_Index),'_',Img_Folder(j).name,'.jpg'));
%             val_Index=val_Index+1;
%         elseif ismember(i,test)
%             imwrite(Img,strcat('./data/test/',num2str(test_Index),'_',Img_Folder(j).name,'.jpg'));
